...
import com.merchant.service.StoreService;
...
public class StoreResource {
    ...
    @GetMapping("/{storeId}/table/{tableId}/available")
    public ResponseEntity<ApiResponse<Boolean>> checkStoreAndTableAvailability(
            @PathVariable(name = "storeId") final String storeId,
            @PathVariable(name = "tableId") final String tableId
    ) {
        var res = storeService.isTableAvailable(storeId, tableId);
        return ApiResponse.<Boolean>build().withData(res).toEntity();
    }
